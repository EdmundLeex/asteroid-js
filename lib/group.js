(function(root){
  ...
  var _randomPos = function (game, maxSize) {
	...
	var _offScreen = function (game, maxSize) {
		var randPos = _randomPos(game, maxSize);
	...
	var _ensureDistanceFromShip = function (game) {
		var pos = _randomPos(game, 0);
		while (Asteroids.Util.calcDist(pos, game.ship.pos) < 50) {
			pos = _randomPos(game, 0);
		}
		return pos;
	};
	...
	root.Asteroids.FormGroup = function (form) {
		...
		var randomSpawn = function () {
			...
			for (var i = 0; i < MAX_SIZE / 2; i++) {
				...
				asteroid.pos = _ensureDistanceFromShip(game);

(function(root){
	...
	root.Asteroids.FormGroup = function (form, asteroids) {
		var game = this;
		var MAX_SIZE = 30;
		...
		var _randomPos = function (game) {
			var randIdx= Math.floor(Math.random() * 2);
			var randX = Math.floor(Math.random() * (game.width - MAX_SIZE));
			var randY = Math.floor(Math.random() * game.height - MAX_SIZE);
